...
export function BookmarkIconButton({ isSaved = false, onClick }) {
  const handleClick = (e) => {
    e.preventDefault();
    e.stopPropagation();
    onClick?.(e);
  ...
  return (
    <div className="relative flex items-center justify-center">
      ...
      >
        ...
        >
          <Bookmark className="opacity-60 text-charcoal" size={16} aria-hidden="true" />

          <Bookmark
            className="absolute inset-0 text-[#111111] fill-[#111111] transition-all duration-300"
            size={16}
            aria-hidden="true"
          ...
          <AnimatePresence>
            {isSaved && (
              <motion.div
                ...
                style={{
                  background:
                    "radial-gradient(circle, rgba(17,17,17,0.4) 0%, rgba(17,17,17,0) 80%)",
                }}
                {...animations.burst}
              />
            )}
